...
if (isset($_POST['submit'])) {
	...
	if($f_verify_userid['Total'] > 0){
		echo "<script>window.location='borangseminar.php?msg=duplicate';</script>";
	}else{
		...
		$chk="";  	
			...
			foreach ($checkbox1 as $chk1){  
				...
				if ($resultCheck > 1) {
					header("Location: borangseminar.php?borangseminar=usertaken");
					exit();
				} 
				else {
					...
					$sql=mysqli_query($conn,"insert into user
							...
							// echo'<script>alert("Pengesahan pendaftaran  tuan/puan untuk mengikuti Seminar Pengguna GPKI telah berjaya. Sila rujuk penyesahan yang akan dihantar ke e-mel tuan/puan dalam masa yang terdekat. Jika tidak mendapat sebarang pengesahan/makluman selepas 1 jam dari notifikasi ini, sila hubungi pihak urusetia seperti didalam agenda.")</script>';    
							$body1 =  mysqli_query($conn,"select * from email2 ORDER BY id_email2 ASC");
							$body1 = mysqli_fetch_array($body1);
							$subject1 =  mysqli_query($conn,"select * from email2 ORDER BY id_email2 ASC");
							$subject1 = mysqli_fetch_array($subject1);
							$altbody1 =  mysqli_query($conn,"select * from email2 ORDER BY id_email2 ASC");
							...
							echo "<script>window.location='keputusan.php';</script>";
								    ...
								    //Recipients
								    $mail->setFrom('akmalhafizyakuza@gmail.com', 'Seminar GPKI');
									$mail->addAddress($email1,$nama1);     // Add a recipient
									
									$a= $body1['body_email2'];
									$b= $subject1['subject_email2'];
									$c= $altbody1['altbody_email2'];
